...
package org.springframework.osgi.rfc124.iandt;
 ...
 * 
 */
public abstract class BaseRFC124IntegrationTest extends AbstractConfigurableBundleCreatorTests {
		...
		private Resource parse(String id, Resource resource) {
			if (id.indexOf(SPRING_DM_PREFIX) > -1) {
				try {
					...
					BaseRFC124IntegrationTest.this.logger.info("Using clover instrumented jar " + res.getDescription());
					return res;
				}
				catch (Exception ex) {
	...
	protected String[] getBundleContentPattern() {
		...
		String[] patterns = new String[] {
			BaseRFC124IntegrationTest.class.getName().replace('.', '/').concat("*.class"), pkg + "**/*" };
		return patterns;
	}
