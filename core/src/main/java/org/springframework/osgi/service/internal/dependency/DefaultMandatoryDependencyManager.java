 ...
 */
package org.springframework.osgi.service.internal.dependency;
...
public class DefaultMandatoryDependencyManager implements MandatoryServiceDependencyManager, BeanFactoryAware,
	...
	private class ImportersListeners implements MandatoryDependencyListener {
		...
		public void mandatoryServiceSatisfied(MandatoryDependencyEvent event) {
			boolean trace = log.isTraceEnabled();

			ServiceImporter importer = event.getServiceImporter();
			for (Iterator iter = ((List) importerToExportersDeps.get(importer)).iterator(); iter.hasNext();) {
				ServiceExporter exporter = (ServiceExporter) iter.next();
		...
		public void mandatoryServiceUnsatisfied(MandatoryDependencyEvent event) {
			boolean trace = log.isTraceEnabled();
			ServiceImporter importer = event.getServiceImporter();
	...
	public void addServiceExporter(String exporterBeanName) {
		...
		if (exportersSeen.putIfAbsent(exporterBeanName, VALUE) == null) {
			...
			if (!beanFactory.isSingleton(beanName))
				...
				ServiceExporter exporter = (ServiceExporter) beanFactory.getBean(beanName);
	 ...
	 */
	protected void discoverDependentImporterFor(String exporterBeanName, ServiceExporter exporter) {
		...
		for (int i = 0; i < importerNames.length; i++) {
			if (beanFactory.isSingleton(importerNames[i])) {
				ServiceImporter importer = (ServiceImporter) beanFactory.getBean(importerNames[i]);
		...
		for (Iterator iter = imps.iterator(); iter.hasNext();) {
			ServiceImporter importer = (ServiceImporter) iter.next();
		...
		synchronized (exporter) {
			...
			for (Iterator iter = imps.iterator(); iter.hasNext();) {
				ServiceImporter imp = (ServiceImporter) iter.next();
