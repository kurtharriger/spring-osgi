...
public class AbstractIoTest extends AbstractConfigurableBundleCreatorTests {
	...
	private final static String PACKAGE = "org/springframework/osgi/iandt/io/";
	...
	public void testClassPathFileLevelWildcardMatching() throws Exception {
		// find all classes
		Resource[] res = patternLoader.getResources("classpath:/" + PACKAGE + "AbstractIoTest.class");
	...
	public void testFileLevelWildcardMatching() throws Exception {
		// find all classes
		Resource[] res = patternLoader.getResources("bundle:/" + PACKAGE + "*.class");
		// at least two integration tests are available
	...
	public void testFileLevelPatternMatching() throws Exception {
		...
		Resource[] res = patternLoader.getResources("bundle:/" + PACKAGE + "AbstractIo*.class");
		// should find only 1
		assertEquals(1, res.length);
	}

	public void testFileLevelCharPatternMatchingForOneChar() throws Exception {
		Resource[] res = patternLoader.getResources("bundle:" + PACKAGE + "AbstractIoTe*t.class");
		// should find only 1
		assertEquals(1, res.length);
	}

	// works on Equinox, Felix | fails on KF (which doesn't return
	// AbstractIoTest.class)
	...
	public void testFileLevelDoubleCharMatching() throws Exception {
		Resource[] res = patternLoader.getResources("bundle:" + PACKAGE + "AbstractIoT??t.class");
		// should find only 1
		assertEquals(1, res.length);
	}
